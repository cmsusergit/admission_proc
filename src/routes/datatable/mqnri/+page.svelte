<script>
    ...
    const changePaymentStatus=async(vv,record)=>{
        const { data, error } = await supabase
        .from('MQNRIFormInfo')
        .update({ is_payment_done: vv })
        .eq('id', record.id)
        .select()
        console.log(data)
    }
                        ...
                        {#if role=='admin'}
                            <div class="flex justify-center space-x-2 items-center">
                                ...
                                {#if record.admission_status!=2}
                                    ...
                                    <input checked={record.is_payment_done} on:change={(ee)=>{changePaymentStatus(ee.target.checked,record)}} type="checkbox" class="checkbox w-8 h-8 rounded" />
                                {:else}
                                    Admission Cancelled
                                {/if}
                            </div>
